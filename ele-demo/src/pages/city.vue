<template>
    <div class="city_container">
        ...
        <ul class="getpois_ul">
            <li v-for="(item,index) in playlist" :key="index" @click="nextpage(index,item.geohash)">
                <h4 class="pois_name ellipsis">{{item.name}}</h4>
                <p class="pois_address ellipsis">{{item.address}}</p>
            </li>
        </ul>
        <div class="search_none_place" v-if="!playlist.length">很抱歉 无搜索结果</div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        async postpois () {
            if(this.inputValue) {
                this.playlist = await searchplace(this.cityid,this.inputValue)
                // console.log(data)
            }
        },
