    ...
    <div>
        <head-top>
            <router-link to="/search/geohash" class="link_search" slot="search">
                <svg width="100%" height="100%" xs:xmlns="http//www.w3.org/2000/svg" version="1.1">
                    ...
                    <div class="swiper-slide food_types_container" v-for="(item, index) in foodTypes" :key="index">
                        <router-link v-for="foodItem in item" :key="foodItem.id" class="link_to_food" :to="{
                            ...
                            query: {
                                ...
                                restraurant_category_id: foodItem.link
                            }
                        }">
                            <figure>
...
export default {
    ...
    async mounted () {
        ...
        let resArr = [...data];
        let foodArr = [];
        for(let i = 0, j = 0; i < resLength; j++, i += 8) {
            foodArr[j] = resArr.splice(0,8)
        }
        this.foodTypes = foodArr;
        // MVVM 数据改变 到界面更新 异步操作 vnode domnode 
